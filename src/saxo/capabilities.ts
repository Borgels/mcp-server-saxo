...
export const SAXO_CAPABILITIES: SaxoCapability[] = [
  ...
  },
  {
    ...
    safetyNotes: [
      ...
      'Returns riskAnalytics with realized-volatility expected moves, vol-scaled stop/target levels, touch probabilities, expected days-to-touch estimates, and simple model EV when payoff inputs are available.',
  ...
  },
  {
    ...
    safetyNotes: [
      ...
      'Selected option candidates include playbook-derived followUpRules for profit taking, soft vol stops, time stops, and trim-vs-close review thresholds.',
  ...
  },
  {
    ...
    examples: [
      ...
      },
      {
        ...
        defaultRules: {
          ...
          hardStopPercentOfEntryValue: 30,
          softStopSpot: 205,
          timeStopDte: 30,
    ...
    safetyNotes: [
      ...
      'Stock reviews use entry cost/price rules; option reviews additionally use Greeks, theta, expiry, roll, underlying-price thesis invalidation, vol-scaled stops, time stops, and max-profit/max-risk rules when supplied.',
      ...
      'Profit-target reviews include DTE fraction elapsed, profit velocity, suggested trim size, stop-raise level, and indicative ToClose multi-leg trim drafts when contract counts allow it.',
  ...
  {
    id: 'saxo_list_price_alerts',
    title: 'List Price Alerts',
    description: 'List Saxo price alert definitions for the current user, optionally filtered by state.',
    risk: 'read',
    examples: [{ state: 'Enabled', top: 50 }],
    identifierFormats: ['Optional State: Enabled|Disabled|RecentlyTriggered'],
    safetyNotes: ['Read-only.'],
    keywords: ['price alert', 'alerts', 'alarm', 'notification', 'watch'],
  },
  {
    id: 'saxo_get_price_alert',
    title: 'Get Price Alert',
    description: 'Fetch a specific Saxo price alert definition by AlertDefinitionId.',
    risk: 'read',
    examples: [{ alertDefinitionId: '30834' }],
    identifierFormats: ['AlertDefinitionId'],
    safetyNotes: ['Read-only.'],
    keywords: ['price alert', 'alerts', 'details'],
  },
  {
    id: 'saxo_create_price_alert',
    title: 'Create Price Alert',
    description:
      'Create a Saxo price alert definition for a Uic/AssetType/AccountId target. Use instrument and account lookup tools first.',
    risk: 'write',
    examples: [
      {
        AccountId: '13457INET',
    ...
    identifierFormats: ['AccountId + Uic + AssetType'],
    safetyNotes: [
      'Creates a Saxo-side notification, not an order.',
      'LIVE alert writes require SAXO_ENABLE_LIVE_ALERT_WRITES=true + policy.allow_live_alert_writes.',
    ],
    keywords: ['create price alert', 'alert', 'alarm', 'notification', 'target price'],
  },
  {
    id: 'saxo_update_price_alert',
    title: 'Update Price Alert',
    description:
      'Update an existing Saxo price alert. Partial input is merged with the current definition before PUT.',
    risk: 'write',
    examples: [{ AlertDefinitionId: '30834', State: 'Disabled' }],
    identifierFormats: ['AlertDefinitionId plus fields to change'],
    safetyNotes: ['Same LIVE alert-write guards as saxo_create_price_alert.'],
    keywords: ['update price alert', 'disable alert', 'enable alert', 'modify alert'],
  },
  {
    id: 'saxo_delete_price_alerts',
    title: 'Delete Price Alerts',
    description: 'Delete one or more Saxo price alert definitions.',
    risk: 'write',
    examples: [{ alertDefinitionIds: ['30834'] }],
    identifierFormats: ['AlertDefinitionId[]'],
    safetyNotes: ['Same LIVE alert-write guards as saxo_create_price_alert.'],
    keywords: ['delete price alert', 'remove alert', 'cancel alert'],
  },
  {
    id: 'saxo_get_price_alert_user_settings',
    title: 'Get Price Alert Notification Settings',
    description: 'Read Saxo price-alert notification settings such as email, popup, and sound.',
    risk: 'read',
    examples: [{}],
    identifierFormats: [],
    safetyNotes: ['Read-only.'],
    keywords: ['price alert settings', 'notification settings', 'email', 'popup', 'sound'],
  },
  {
    id: 'saxo_update_price_alert_user_settings',
    title: 'Update Price Alert Notification Settings',
    description:
      'Update Saxo price-alert notification settings. Partial input is merged with current settings before PUT.',
    risk: 'write',
    examples: [{ NotifyWithMail: true, NotifyWithPopup: true, Sound: 'None' }],
    identifierFormats: ['EmailAddress, NotifyWithMail, NotifyWithPopup, Sound'],
    safetyNotes: [
      'Changes notification settings, not orders.',
      'LIVE alert writes require SAXO_ENABLE_LIVE_ALERT_WRITES=true + policy.allow_live_alert_writes.',
    ],
    keywords: ['update notification settings', 'price alert email', 'popup', 'sound'],
  },
  {
